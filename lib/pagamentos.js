...
export async function processarWebhookPagBank(evento) {
  ...
  const pedidosAntes = await sql`
    SELECT *
    FROM pedidos
    WHERE id = ${pedidoId} AND provedor = 'pagbank'
  `;
  const pedidoExistente = pedidosAntes[0];
  if (!pedidoExistente) return { ok: false, motivo: 'pedido pagbank nao encontrado' };

  if (
    evento.id &&
    pedidoExistente.provedor_preferencia_id &&
    String(evento.id) !== String(pedidoExistente.provedor_preferencia_id)
  ) {
    return { ok: false, motivo: 'checkout nao confere com o pedido' };
  }

  const valorEvento = Number(charge?.amount?.value ?? evento.items?.[0]?.unit_amount);
  if (status === 'aprovado' && valorEvento !== Number(pedidoExistente.valor_centavos)) {
    return { ok: false, motivo: 'valor pago nao confere com o pedido' };
  }
